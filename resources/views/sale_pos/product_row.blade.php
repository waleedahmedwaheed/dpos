<tr class="product_row" data-row_index="{{$row_count}}">
	...
	<td class="text-center v-center">
		...
		@endphp
		<input type="{{$subtotal_type}}" class="form-control pos_line_total @if(!empty($pos_settings['is_pos_subtotal_editable'])) input_number @endif" value="{{@num_format($product->quantity_ordered*$unit_price_inc_tax - (($product->product_to*$product->quantity_ordered*$product->weight) + ($product->product_ato*$product->quantity_ordered*$product->weight)) )}}">
		<span class="display_currency pos_line_total_text @if(!empty($pos_settings['is_pos_subtotal_editable'])) hide @endif" data-currency_symbol="true">{{(($product->product_to*$product->quantity_ordered*$product->weight) + ($product->product_ato*$product->quantity_ordered*$product->weight))}}</span>
	</td>
