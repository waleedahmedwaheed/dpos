  ...
  <div class="table-responsive">
    <table class="table table-bordered add-product-price-table table-condensed {{$class}}">
        ...
        <tr>
          <td>
            <div class="col-sm-4">
              {!! Form::label('product_custom_field1', trans('product.list_price') . ':*') !!}

              {!! Form::text('product_custom_field1', $default, ['class' => 'form-control input-sm dpp_list_price input_number', 'placeholder' => 'List Price', 'required']); !!}
            </div>

            <div class="col-sm-4">
              {!! Form::label('single_dpp', trans('product.exc_of_tax') . ':*') !!}

              {!! Form::text('single_dpp', $default, ['class' => 'form-control input-sm dpp input_number', 'placeholder' => 'Excluding FED', 'required']); !!}
            </div>

            <div class="col-sm-4">
              {!! Form::label('single_dpp_inc_tax', trans('product.inc_of_tax') . ':*') !!}
            
              {!! Form::text('single_dpp_inc_tax', $default, ['class' => 'form-control input-sm dpp_inc_tax input_number', 'placeholder' => 'Including FED', 'required']); !!}
            </div>
          </td>
