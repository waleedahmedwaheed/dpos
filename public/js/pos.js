$(document).ready( function(){
	...
	$('table#pos_table tbody').on('change', 'input.pos_quantity', function(){
        ...
        var weight = __read_number(tr.find('input.weight'));
        var product_to = __read_number(tr.find('input.product_to'));
        var product_ato = __read_number(tr.find('input.product_ato'));
		...
		var line_total = entered_qty * unit_price_inc_tax - ((quantity * weight * product_to) + (quantity * weight * product_ato) + (quantity * weight * row_discount_amount));
	...
	//to amount//
    $('table#pos_table tbody').on('change', 'input.product_to', function(){

        var product_to = __read_number($(this));
        ...
        //var unit_price_inc_tax = __add_percent(discounted_unit_price, tax_rate);
        var line_total = (quantity * pos_unit_price) - ((product_to * quantity * weight)+(product_ato * quantity * weight)+(row_discount_amount * quantity * weight));
        //__write_number(tr.find('input.pos_unit_price_inc_tax'), unit_price_inc_tax);
        __write_number(tr.find('input.pos_line_total'), line_total, false, 2);
        tr.find('span.pos_line_total_text').text(__currency_trans_from_en(line_total, true));
        pos_each_row(tr);
    ...
    $('table#pos_table tbody').on('change', 'input.product_ato', function(){

        var product_ato = __read_number($(this));
	...
	$('table#pos_table tbody').on('change', 'input.row_discount_amount',
	function(){
		var tr = $(this).parents('tr');

		//calculate discounted unit price
		var discounted_unit_price = calculate_discounted_unit_price(tr);
        var entered_amount = __read_number($(this));
		var tax_rate = tr.find('select.tax_id').find(':selected').data('rate');
        ...
        var unit_price_inc_tax = __read_number(tr.find('input.pos_unit_price_inc_tax'));
        var line_total = quantity * unit_price_inc_tax - ((quantity * weight * product_to) + (quantity * weight * product_ato) + (quantity * weight * entered_amount));
	...
	//Datetime picker
    $('#transaction_date,#transaction_dates,#transaction_date_edit').datetimepicker({
...
function pos_each_row(row_obj){
	...
	var product_to = __read_number(row_obj.find('input.product_to'));
	...
	var unit_price_inc_tax = unit_price;
	__write_number(row_obj.find('input.pos_unit_price_inc_tax'), unit_price);
