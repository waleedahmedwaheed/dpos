$(document).ready( function(){
    ...
    if($('#purchase_sale_date_filter').length == 1){
        $('#purchase_sale_date_filter').daterangepicker(
            dateRangeSettings,
            function (start, end) {
                $('#purchase_sale_date_filter span').html(start.format(moment_date_format) + ' ~ ' + end.format(moment_date_format));
                updatePurchaseSale();
            }
        );
        $('#purchase_sale_date_filter').on('cancel.daterangepicker', function(ev, picker) {
            $('#purchase_sale_date_filter').html('<i class="fa fa-calendar"></i> ' + LANG.filter_by_date);
        });
        updatePurchaseSale();
    }
    $('#purchase_sale_date_filter').change( function(){
        updatePurchaseSale();
    });
...
function updatePurchaseSale(){

    var start = $('#purchase_sale_date_filter').data('daterangepicker').startDate.format('YYYY-MM-DD');
    var end = $('#purchase_sale_date_filter').data('daterangepicker').endDate.format('YYYY-MM-DD');
    var location_id = $('#purchase_sale_location_filter').val();
    ...
    $.ajax({
        method: "GET",
        url: '/reports/product-purchase-sale',
        dataType: "json",
        data: data,
        success: function(data){
            console.log(data.opening_stock);
            var opening_stock_td = $("#opening_stock");

            $.each(data.opening_stock, function(i, item) {
                // console.log(data.opening_stock[i].stock);
                opening_stock_td.append("<li> Id: " + data.opening_stock[i].product_id + "</li>");
                opening_stock_td.append("<li> Stock: " + data.opening_stock[i].stock + "</li>");
            });
            // $('.opening_stock').html(__currency_trans_from_en( data.opening_stock, true ));
            // $('.closing_stock').html(__currency_trans_from_en( data.closing_stock, true ));
            // $('.total_sell').html(__currency_trans_from_en( data.total_sell, true ));
