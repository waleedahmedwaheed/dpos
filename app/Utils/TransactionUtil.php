...
//                    'line_discount_type' => !empty($product['line_discount_type']) ? $product['line_discount_type'] : null,
                    ...
                    'product_to' => $this->num_uf($product['product_to']),
                    'product_ato' => $this->num_uf($product['product_ato'])
                ];
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        $output = [
            ...
            'table_unit_price_tax_label' => $il->table_unit_price_tax_label,
            'table_subtotal_label_tax' => $il->table_subtotal_label_tax,
            'tax_label' => $il->tax_label,
            'product_to' => $il->product_to,
            'product_ato' => $il->product_ato
        ];
    ...
    protected function _receiptDetailsSellLines($lines, $il, $is_product_expiry_enabled, $is_lot_number_enabled)
    {
        ...
        foreach ($lines as $line) {
            ...
            $default_purchase_price = Variation::find($variation->id)->default_purchase_price;
            ...
            $line_array = [
                ...
                'product_to' => $this->num_f($line->product_to),
                'product_ato' => $this->num_f($line->product_ato),
                'line_discount_amount' => $this->num_f($line->line_discount_amount),
                'unit_price' => $this->num_f($line->unit_price),
                'tax' => ($variation->default_purchase_price * ($tax_details->amount/100)) * $line->quantity,
                ...
                'price_exc_tax' => $this->num_f($line->quantity * $line->unit_price),
            ...
            if (!empty($line->modifiers)) {
                foreach ($line->modifiers as $modifier_line) {
                    ...
                    $modifier_line_array = [
                        ...
                        'price_exc_tax' => $this->num_f($modifier_line->quantity * $modifier_line->unit_price),
