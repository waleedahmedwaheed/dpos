...
class ReportController extends Controller
{
    ...
    public function getProductPurchaseSale(Request $request)
    {
        if (!auth()->user()->can('profit_loss_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($request->ajax()) {
            ...
            $day_before_start_date = \Carbon::createFromFormat('Y-m-d', $start_date)->subDay()->format('Y-m-d');
            //Get Opening stock
            $opening_stock = $this->transactionUtil->getOpeningClosingStockProduct($business_id, $day_before_start_date, $location_id, true);
...
//                                + $data['total_purchase_return'] - $data['total_sell_return'];
        ...
        $business_locations = BusinessLocation::forDropdown($business_id, true);
        return view('report.product_purchase_sale_stock', compact('business_locations','opening_stock'));
    }
...
//                ->where('products.id', $product_id);
        ...
        $product_details = $query->select(
            DB::raw("(SELECT COALESCE(SUM(IF(transactions.type='sell', TSL.quantity - TSL.quantity_returned , -1* TPL.quantity) ),0) FROM transactions 
                        LEFT JOIN transaction_sell_lines AS TSL ON transactions.id=TSL.transaction_id
                        LEFT JOIN purchase_lines AS TPL ON transactions.id=TPL.transaction_id
                        WHERE transactions.status='final' AND transactions.type='sell' $location_filter 
                        AND (TSL.product_id=products.id OR TPL.product_id=products.id)) as total_sold"),
            DB::raw("(SELECT COALESCE(SUM(IF(transactions.type='sell_transfer', TSL.quantity, 0) ),0) FROM transactions 
                        ...
                        WHERE transactions.status='final' AND transactions.type='sell_transfer' $location_filter 
                        AND (TSL.product_id=products.id)) as total_transfered"),
            DB::raw("(SELECT COALESCE(SUM(IF(transactions.type='stock_adjustment', SAL.quantity, 0) ),0) FROM transactions 
                        LEFT JOIN stock_adjustment_lines AS SAL ON transactions.id=SAL.transaction_id
                        WHERE transactions.status='received' AND transactions.type='stock_adjustment' $location_filter 
                        AND (SAL.product_id=products.id)) as total_adjusted"),
            ...
            DB::raw("(SELECT COALESCE(SUM(
                            purchase_lines.quantity),0) 
                             from purchase_lines 
                            inner join transactions as purchase on purchase_lines.transaction_id = purchase.id
                         where purchase.business_id = $business_id and purchase.type = 'opening_stock' and
                          purchase_lines.product_id = products.id AND
                          (date(transaction_date) <= '$date' or date(transaction_date) = '$next_day' 
                        )) as opening_stock"),
            DB::raw("(SELECT COALESCE(SUM(
                            ...
                            WHERE  
                            tspl.purchase_line_id = purchase_lines.id AND 
                            date(sale.transaction_date) <= '$date') )),0) 
                            from purchase_lines inner join transactions as purchase on purchase_lines.transaction_id = purchase.id
                         where purchase.business_id = $business_id and
                          purchase_lines.product_id = products.id AND
                          (date(transaction_date) <= '$date') 
                        ) as closing_stock"),
            DB::raw("(SELECT COALESCE(SUM(purchase_lines.quantity),0) from purchase_lines 
            inner join transactions as t on purchase_lines.transaction_id = t.id 
            where t.business_id = 1 and t.type = 'purchase' and t.status='received' AND
              purchase_lines.product_id = products.id ) as purchase"),
            'sku',
            'products.name as product',
            'products.type',
            'units.short_name as unit',
            'products.unit_id as unitid',
            'products.weight as weight',
            'products.enable_stock as enable_stock',
            'products.id as DT_RowId',
            'V.dpp_inc_tax as unit_price'
