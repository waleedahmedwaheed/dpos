...
//            }
            ...
            if ($request->hasFile('business_logo') && $request->file('business_logo')->isValid()) {
                if ($request->business_logo->getSize() <= config('constants.image_size_limit')) {
                    $new_file_name = time() . '_' . $request->business_logo->getClientOriginalName();
                    $path = publicPath('/business_logos');
                    if (!File::isDirectory($path)) { // path does not exist
                        File::makeDirectory($path, 0777, true, true); // create directory
                    }
                    $request->business_logo->move($path, $new_file_name);
                    if ($path) {
                        $business_details['logo'] = $new_file_name;
                    }
                }
            }
